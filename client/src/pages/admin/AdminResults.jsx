...
export default function AdminResults() {
    ...
    const downloadIndividualPDF = (student) => {
        ...
        doc.setFontSize(20);
        doc.text(exam.title || 'Exam Scorecard', 105, 18, { align: 'center' });
        
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text(`EXAM SCORECARD | DURATION: ${exam.duration_minutes || 180} MINS`, 105, 28, { align: 'center' });
        ...
        const detailsData = [
            ['Student Name:', student.studentName || 'N/A', 'Exam Type:', exam.examType || 'N/A'],
            ['Email:', student.studentEmail || 'N/A', 'Submission:', student.endTime ? new Date(student.endTime).toLocaleString('en-IN') : 'N/A'],
            ['Correct Answers:', String(student.correct || 0), 'Incorrect Answers:', String(student.incorrect || 0)],
            ['Unattempted:', String(student.unattempted || 0), 'Final Marks Obtained:', `${student.score} / ${(student.totalQuestions || exam.questions?.length || 0) * 4}`]
        ];
        ...
        const weakData = student.weakAreas && student.weakAreas.length > 0
            ? student.weakAreas.map(w => [w.chapter || 'N/A', `${w.incorrect} Incorrect Answers`])
        ...
        // Save file
        doc.save(`${student.studentName || 'Scorecard'}_Result.pdf`);
    };
